...
var File = require('../models/file_model');         // Cargo el modelo de archivos de la BD
...
var TeamController = {
    ...
    saveMember: function(req,res){
        ...
        var values = req.body;
        //console.log("Picture ID: " + values.picture);
        member.name = values.name;
        member.position = values.position;
        member.description = values.description;
        member.picture = values.picture; 
        member.slider_order = values.slider_order;
        member.show = values.show;
    ...
    deleteMember: function(req,res){
        ...
        TeamMember.findByIdAndDelete(memberID, (err, memberDeleted) => {
            ...
            File.findByIdAndDelete(memberDeleted.picture, (err, fileDeleted) => {
                //console.log(fileDeleted);
                if(err) return res.status(500).send({message: 'No se ha podido Borrar la imágen de la BD'});
                if(!fileDeleted){
                    return res.status(404).send({message: 'No existe la imágen en la BD'});
                } 
                else{
                    ...
                    fs.exists(deleteFile, (exists) => {
                        if(exists){
                            fs.unlink(deleteFile, (error) => {
                                if(error) return res.status(500).send({message: 'Error al borrar el archivo de imágen'});
                                //else return res.status(200).send({file: fileDeleted});
                            });
                        }
                        else return res.status(404).send({message: 'No existe el archivo de imágen en el servidor'});
                    });
                }
            });

            if(err) return res.status(500).send({message: 'No se ha podido Borar el miembro de la BD'});
